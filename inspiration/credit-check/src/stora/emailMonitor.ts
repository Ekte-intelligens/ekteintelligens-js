 ...
 */
export const storeEmail = (organizationId: string, email: string): void => {
    try {
 ...
 */
export const initEmailMonitor = (config: StoraCreditCheckConfig): void => {
 ...
 */
const setupEmailMonitoring = (config: StoraCreditCheckConfig): void => {
    const emailInput = document.querySelector<HTMLInputElement>('#order_form_email');
    
    if (!emailInput) {
        // Email input not found, might not be on the right page
        // Try again after a short delay in case it's loaded dynamically
        setTimeout(() => {
            const retryInput = document.querySelector<HTMLInputElement>('#order_form_email');
            if (retryInput) {
...
const attachEmailListener = (
    ...
    if (emailInput.value) {
        storeEmail(config.organization_id, emailInput.value);
    }
    ...
    console.log('Email monitoring initialized for', config.organization_id);
};
