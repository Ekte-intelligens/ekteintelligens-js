import type { StoraCreditCheckConfig } from '../stora-integration.ts';
import { getStoredEmail } from './emailMonitor.ts';
import { createCreditCheckSession, checkCreditCheckStatus } from '../services/creditCheckSession.ts';
import { showCreditCheckDialog } from './dialog.ts';
import type { CreditCheckSession } from '../services/creditCheckSession.ts';

/**
 * Initialize booking complete page handler
 */
export const initBookingComplete = (config: StoraCreditCheckConfig): void => {
 ...
 */
const setupBookingComplete = async (config: StoraCreditCheckConfig): Promise<void> => {
    // Check if we're on the booking complete page
    const bookingComplete = document.querySelector<HTMLElement>('.booking-complete');
    ...
    if (!email) {
        console.warn('No email found in localStorage for credit check');
    ...
    try {
        ...
        if (email) {
            try {
                session = await createCreditCheckSession({
                    ...
                    integration_subscriber_id: config.integration_subscriber_id,
                    email: email,
                });
                console.log('Credit check session initialized:', session);
            } catch (error) {
                console.error('Failed to initialize credit check session:', error);
            }
        }
        ...
        if (session) {
            ...
            if (existingStatus && (
                ...
                console.log('Credit check already passed, showing default content');
                return;
            }
        }

        // Show credit check dialog
        const actionsSection = bookingComplete.querySelector<HTMLElement>('.booking-complete__actions_section');
        if (actionsSection) {
            showCreditCheckDialog(bookingComplete, {
                organizationId: config.organization_id,
                onCreditCheckComplete: (completedSession) => {
                    console.log('Credit check completed:', completedSession);
            ...
            console.warn('Could not find booking-complete__actions_section');
        }
    } catch (error) {
